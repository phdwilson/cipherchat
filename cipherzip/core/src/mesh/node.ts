...
export interface ShardMeta {
  ...
  size: number
  storedAt: number
  owners: string[] // nodeIds known to hold this shard
}
...
export class MeshStorage {
  ...
  private shards = new Map<string, ShardMeta>()
  ...
  async init() {
    ...
    try {
      ...
      const j = JSON.parse(raw) as { shards: ShardMeta[] }
      for (const s of j.shards || []) this.shards.set(s.hash, s)
    } catch {
      /* fresh */
    }
  }
  ...
  async persist() {
    await writeFile(
      this.metaPath,
      JSON.stringify({ nodeId: this.nodeId, shards: [...this.shards.values()] }, null, 2)
    )
  }
  ...
  /** 切分并本地存储（返回分片哈希列表） */
  async putObject(data: Buffer, chunkSize = 256 * 1024): Promise<string[]> {
    const hashes: string[] = []
  ...
  async getObject(hashes: string[]): Promise<Buffer> {
    const parts: Buffer[] = []
    for (const h of hashes) {
      const p = await this.loadShard(h)
      if (!p) throw new Error(`缺失分片: ${h}`)
      parts.push(p)
    }
    return Buffer.concat(parts)
  }

  async storeShard(hash: string, data: Buffer): Promise<boolean> {
    if (!this.willing && this.shards.size > 0) {
      // 仍允许存自己的对象
    }
    const free = await this.freeBytes()
    if (data.length > free) return false
    const path = join(this.shardDir, hash)
    await writeFile(path, data)
    const meta: ShardMeta = this.shards.get(hash) || {
      hash,
      size: data.length,
      storedAt: Date.now(),
      owners: [this.nodeId],
    }
    if (!meta.owners.includes(this.nodeId)) meta.owners.push(this.nodeId)
    this.shards.set(hash, meta)
    await this.persist()
    return true
  }

  async loadShard(hash: string): Promise<Buffer | null> {
    try {
   ...
   * （实际网络复制由 P2P 层完成）
   */
  async healPlan(): Promise<Array<{ hash: string; need: number }>> {
    const plan: Array<{ hash: string; need: number }> = []
    const now = Date.now()
    // 清理过期 peer（5 分钟无心跳）
    for (const [id, p] of this.peers) {
      if (now - p.lastSeen > 5 * 60_000) this.peers.delete(id)
    }
  ...
  info(): MeshNodeInfo & { shardCount: number } {
    return {
      nodeId: this.nodeId,
      address: '127.0.0.1',
      port: 0,
