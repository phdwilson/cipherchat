...
import { CCZ_MAGIC, CCZ_VERSION, ArchiveFlags, CipherAlgo, CompressAlgo, DEFAULT_CHUNK_SIZE, } from '@cipherzip/shared';
import { deriveKeys, sha256Buf } from '../crypto/kdf.js';
...
async function collectFiles(paths, baseDir) {
    const out = [];
 ...
 * 创建 .ccz 归档（强制 E2E）
 */
export async function createCcz(opts) {
    ...
    if (opts.encryptFilenames !== false)
        flags |= ArchiveFlags.ENCRYPT_FILENAMES;
    if (opts.key.type === 'keyfile' || opts.key.type === 'hybrid')
        flags |= ArchiveFlags.KEYFILE_MODE;
    ...
    for (const f of files) {
        ...
        const displayPath = flags & ArchiveFlags.ENCRYPT_FILENAMES ? sealName(keys.nameKey, f.rel) : f.rel;
        const displayName = flags & ArchiveFlags.ENCRYPT_FILENAMES ? sealName(keys.nameKey, basename(f.rel)) : basename(f.rel);
        if (f.isDir) {
            entries.push({
                path: displayPath,
                name: displayName,
        ...
        entries[entries.length - 1].entryId = entryId;
        doneBytes += f.size;
 ...
 * 打开并解析 .ccz（验证密钥）
 */
export async function openCcz(filePath, key) {
    const data = await readFile(filePath);
    return openCczBuffer(data, key);
}
export async function openCczBuffer(data, key) {
    let off = 0;
    const magic = data.subarray(0, 4);
    if (!magic.equals(Buffer.from(CCZ_MAGIC)))
        throw new Error('不是有效的 .ccz 文件（魔数不匹配）');
    off = 4;
    const version = data.readUInt16LE(off);
    off += 2;
    if (version > CCZ_VERSION)
        throw new Error(`不支持的 .ccz 版本: ${version}`);
    const flags = data.readUInt32LE(off);
    off += 4;
    if (!(flags & ArchiveFlags.FORCE_E2E))
        throw new Error('非法归档：缺少强制 E2E 标志');
    const cipher = data.readUInt8(off);
    off += 1;
    const compressAlgo = data.readUInt8(off);
    off += 1;
    off += 2; // reserved
    const chunkSize = data.readUInt32LE(off);
    off += 4;
    const saltLen = data.readUInt16LE(off);
    off += 2;
    const salt = data.subarray(off, off + saltLen);
    off += saltLen;
    const authHashStored = data.subarray(off, off + 32).toString('hex');
    off += 32;
    ...
    const headerLen = data.readUInt32LE(off);
    off += 4;
    const headerEnc = data.subarray(off, off + headerLen);
    off += headerLen;
    const tocLen = data.readUInt32LE(off);
    off += 4;
    ...
    const meta = JSON.parse(metaPlain.toString('utf8'));
    // 覆盖公开头中的字段保证一致
    meta.flags = flags;
    meta.cipher = cipher;
    ...
    const rawEntries = JSON.parse(tocPlain.toString('utf8'));
    const chunkCount = data.readUInt32LE(off);
    off += 4;
    const chunks = [];
    for (let i = 0; i < chunkCount; i++) {
        const cl = data.readUInt32LE(off);
        off += 4;
 ...
 * 解密解压提取到目录
 */
export async function extractCcz(opts) {
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        if (opts.filter && opts.filter.length && !opts.filter.includes(e.path))
            continue;
...
/** 仅列出内容（需密钥） */
export async function listCcz(archivePath, key) {
