 ...
 */
export function seal(key, plain, aad, algo = CipherAlgo.AES_256_GCM) {
    const nonce = randomBytes(NONCE_LEN);
    const name = algoName(algo);
    const cipher = createCipheriv(name, key, nonce);
    if (aad && aad.length)
...
export function open(key, wire, aad, algo = CipherAlgo.AES_256_GCM) {
    ...
    const name = algoName(algo);
    const decipher = createDecipheriv(name, key, nonce);
    if (aad && aad.length)
