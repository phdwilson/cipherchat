...
import { PBKDF2_KEY_ITERS, PBKDF2_AUTH_ITERS, } from '@cipherzip/shared';
 ...
 */
export async function extractKeyfileMaterial(filePath) {
...
async function materialToSecret(material) {
 ...
 */
export async function deriveKeys(material, salt, iters = PBKDF2_KEY_ITERS) {
    ...
    const auth = await pbkdf2(secret, Buffer.concat([Buffer.from('cipherzip:auth:'), s]), PBKDF2_AUTH_ITERS, 32, 'sha256');
    // 清理 secret
    secret.fill(0);
    return {
...
export async function deriveCipherChatCompatible(channelId, password) {
    const auth = await pbkdf2(password, `cipherchat:auth:${channelId}`, PBKDF2_AUTH_ITERS, 32, 'sha256');
    const key = await pbkdf2(password, `cipherchat:key:${channelId}`, PBKDF2_KEY_ITERS, 32, 'sha256');
