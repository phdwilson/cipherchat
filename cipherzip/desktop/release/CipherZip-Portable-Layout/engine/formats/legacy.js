...
async function collectFiles(paths) {
    ...
    async function walk(p) {
        ...
        if (st.isDirectory()) {
            for (const k of await readdir(p))
                await walk(join(p, k));
        }
        else if (st.isFile())
            files.push(p);
 ...
 */
export async function packArchive(opts) {
    ...
    if (format === 'ccz') {
        ...
        if (opts.key)
            key = opts.key;
        else if (opts.keyfilePath && opts.password)
            key = { type: 'hybrid', password: opts.password, keyfilePath: opts.keyfilePath };
        else if (opts.keyfilePath)
            key = { type: 'keyfile', path: opts.keyfilePath };
        else if (opts.password)
            key = { type: 'password', password: opts.password };
        else
            throw new Error('.ccz 格式强制端到端加密，必须提供密码或密钥文件');
        await createCcz({
            inputs: opts.inputs,
            output: opts.output,
            key,
...
async function packTar(opts, format) {
    // 用 archiver tar
    await mkdir(dirname(opts.output), { recursive: true });
    const output = createWriteStream(opts.output);
    ...
    if (format === 'tar.br') {
        ...
        for (const p of opts.inputs) {
            const st = await stat(p);
            if (st.isDirectory())
                tar.directory(p, basename(p));
            else
                tar.file(p, { name: basename(p) });
        }
        await tar.finalize();
        await tarDone;
...
async function packWith7z(opts, format) {
    if (!(await hasBin('7z'))) {
    ...
    if (format === '7z')
        ...
        args.push('-trar'); // 7z 对 rar 创建支持有限
    else if (format === 'xz')
        args.push('-txz');
    if (opts.password)
        args.push(`-p${opts.password}`, '-mhe=on');
    args.push(opts.output, ...opts.inputs);
    await execFileAsync('7z', args, { timeout: 600_000, maxBuffer: 64 * 1024 * 1024 });
    return opts.output;
}
export async function unpackArchive(opts) {
    ...
    if (format === 'ccz') {
        ...
        if (opts.key)
            ...
            throw new Error('打开 .ccz 需要密码或密钥文件');
    ...
    if (format === '7z' || format === 'rar' || format === 'xz') {
        if (!(await hasBin('7z')))
            throw new Error(`解压 ${format} 需要 7z 命令`);
...
export const SUPPORTED_CREATE = ['.ccz', '.zip', '.tar', '.tar.gz', '.tgz', '.tar.br', '.gz', '.7z'];
export const SUPPORTED_OPEN = ['.ccz', '.zip', '.tar', '.tar.gz', '.tgz', '.tar.br', '.gz', '.7z', '.rar', '.xz', '.bz2', '.iso'];
