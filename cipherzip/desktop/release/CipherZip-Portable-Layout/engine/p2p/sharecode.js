 ...
 * 不是服务器发放的 — 完全本地生成。
 */
import { createHash, createHmac, generateKeyPairSync, createPrivateKey, createPublicKey, diffieHellman, } from 'node:crypto';
...
function payloadToBytes(p) {
    ...
    pubHash.copy(buf, 7);
    let caps = 0;
    if (p.caps.includes('chat'))
        caps |= 1;
    if (p.caps.includes('file'))
        caps |= 2;
    if (p.caps.includes('mesh'))
        caps |= 4;
...
function bytesToWords(data20) {
    ...
    let bits = 0n;
    for (const b of full.subarray(0, 22))
...
function wordsToBytes(words) {
    if (words.length !== 16)
        throw new Error('分享码应为 16 个英文单词');
    let bits = 0n;
    for (const w of words) {
        const idx = WORDLIST.indexOf(w.toLowerCase());
        if (idx < 0)
            throw new Error(`未知单词: ${w}`);
    ...
    if (crc !== expect)
        throw new Error('分享码校验失败（可能输入有误）');
    return data20;
}
...
export function decodeShareCode(code) {
    ...
    if (version !== VERSION)
        throw new Error(`不支持的分享码版本: ${version}`);
...
export function encodeShareQr(payload) {
    return JSON.stringify({ ...payload, code: encodeShareCode(payload) });
}
export function decodeShareQr(text) {
    const j = JSON.parse(text);
    if (j.v !== 1 || !j.host || !j.port || !j.pub)
        throw new Error('无效的分享二维码');
    return j;
}
/** 生成 P2P 会话密钥对（X25519） */
export function generateP2PIdentity() {
...
/** 用 HMAC 派生会话传输密钥 */
export function deriveSessionKey(privateKeyDerB64, peerPublicDerB64, salt) {
